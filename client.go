...
func (c *Client) readPump() {
	defer func() {
		...
		msg := []byte(`{"total":` + strconv.Itoa(total) + `, "header":"close", "from":"` + c.id + `"}`)
		tomessage := &BroadCast{
			msg: msg,
			to:  c.room,
		}
...
func serveWs(hub *Hub, c *gin.Context) {
	...
	client := &Client{hub: hub, conn: conn, send: make(chan []byte, 256), id: c.MustGet("auth").(string), room: c.Param("id")}
